...
def enable_nat(bridge, subnet):
    ...
    run_command(
        f"iptables -I FORWARD -i {ext_iface} -d {subnet}.0/24 -m state --state ESTABLISHED,RELATED -j ACCEPT"
    )
...
def insert_network_to_db(user_id, bridge, subnet):
    ...
    try:
        ...
        print(f"[DB] Network inserted for user ID {user_id}")
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print("Usage: sudo python3 create_user_bridges.py <user_id>")
    ...
    if write_netplan_yaml(bridge, subnet) and create_dnsmasq_config(bridge, subnet):
        ...
        insert_network_to_db(user_id, bridge, subnet)
